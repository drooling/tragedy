...
class Mod(commands.Cog, description="Commands to moderate your server !"):
	...
	async def _warns(self, ctx, *, member: commands.MemberConverter):
		...
		try:
			embed = discord.Embed(color=Color.green()).set_author(icon_url=member.avatar_url, name=member)
			warns = list()
			ids = list()
			with self.pool.cursor() as cursor:
				cursor.execute("SELECT * FROM warns WHERE guild=%s AND user=%s", (ctx.guild.id, member.id))
			...
			if not rows:
				...
				for _row in rows:
					...
					warns.append(str("**ID** - `{}` | **Admin** - <@{}> | **Date** - `{}` | **Reason** - `{}`".format(tragedy_warning_id, warner, time, reason)))
				index = 1
				for _warning in warns:
					embed.add_field(name="Warning {}.".format(index), value=_warning, inline=False)
					index += 1

				Confirm = await ctx.reply(embed=embed, mention_author=True, components=[
					[
						Button(style=ButtonStyle.green, label="Clear warnings"),
						Button(style=ButtonStyle.green, label="Remove a warning"),
				...
				try:
					...
					if interaction.component.label == "X":
						...
						await Confirm.delete()
						with self.pool.cursor() as cursor:
							cursor.execute("DELETE FROM warns WHERE guild=%s AND user=%s", (ctx.guild.id, member.id))
						...
						SelectOptions = []
						for _id in ids:
							SelectOptions.append(SelectOption(label=_id, value=_id))
						SelectWarn = await ctx.send(content="Warning Removal", components=[
							[
								Select(
						...
						with self.pool.cursor() as cursor:
							cursor.execute("DELETE FROM warns WHERE guild=%s AND user=%s AND id=%s", (ctx.guild.id, member.id, selectInteraction.component[0].label))
	...
	async def prefix(self, ctx: commands.Context):
		try:
			...
			embed = discord.Embed(title="Prefixes", description='\n'.join(numberedList),
								  color=Color.green())
			embed.set_footer(text="{}/5 Custom Prefixes".format(len(prefixes) - 1))
	...
	async def _add(self, ctx, *, prefix: str):
		try:
			...
			with self.pool.cursor() as cursor:
				cursor.execute("SELECT * FROM prefix WHERE guild=%s", (str(ctx.guild.id)))
				response = dict(cursor.fetchone())
				for col in tragedy.columnNames:
					if response.get(col) is None:
						column = col
						break
			with self.pool.cursor() as cursor:
				cursor.execute(
					"UPDATE prefix SET {0} = %s WHERE guild=%s".format(column), (prefix, str(ctx.guild.id))
					)
	...
	async def _remove(self, ctx, *, prefix: str):
		try:
			...
			if prefix not in prefixes:
				return await ctx.send("That is nto currently a prefix for this server.", delete_after=5)
			if (len(prefixes) - 1) < 1:
			...
			with self.pool.cursor() as cursor:
				...
				response = dict(cursor.fetchone())
				for col in tragedy.columnNames:
					if response.get(col) is prefix:
						column = col
						break
				cursor.execute("UPDATE prefix SET {0}=NULL WHERE guild=%s".format(column), (str(ctx.guild.id)))

...
class Slash(commands.Cog, command_attrs=dict(hidden=True)):
	...
	async def whois(self, ctx, member: discord.Member):
		...
		# userProfile = await member.profile() User profile has been deprecated since v1.7
		...
		# embed.add_field(name="Other Info", value="HypeSquad House - {}\nUser has Nitro - {}\nConnected Accounts - {}".format(str(userProfile.hypesquad_houses).title(), tragedy.EmojiBool(await userProfile.nitro), ', '.join(externalAccounts).removeprefix(', ') if externalAccounts != [] else "None")) User profile has been deprecated since v1.7
	...
	async def serverinfo(self, ctx):
		findbots: int = sum(1 for member in ctx.guild.members if member.bot)
		vanity: bool = "VANITY_URL" in str(ctx.guild.features)
		splash: bool = "INVITE_SPLASH" in str(ctx.guild.features)
		animicon: bool = "ANIMATED_ICON" in str(ctx.guild.features)
		discoverable: bool = "DISCOVERY" in str(ctx.guild.features)
		banner: bool = "BANNER" in str(ctx.guild.features)
		vanityFeature = "{} - Vanity URL".format(
			tragedy.EmojiBool(vanity)) if not vanity else "{} - Vanity URL ({})".format(tragedy.EmojiBool(vanity),
																						await ctx.guild.vanity_url)
		embed = discord.Embed(title='**{}**'.format(ctx.guild.name), color=Color.green(), timestamp=datetime.now())

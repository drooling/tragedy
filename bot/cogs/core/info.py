...
class Info(commands.Cog, description="Commands that return information"):
	...
	async def _emoji(self, ctx, emoji: commands.EmojiConverter):
		...
		embed.add_field(name="Basic Info",
						value="Emoji Name - **{}**\nEmoji ID - **{}**\nCreated At - **{} (<t:{}:R>)**".format(emoji.name,
																										emoji.id,
																										humanize.naturaldate(
																											emoji.created_at.replace(tzinfo=timezone.utc)),
																										int(emoji.created_at.timestamp())))
	...
	@commands.command()
	@commands.cooldown(1, 35, type=BucketType.member)
	async def hibp(self, ctx, *queries):
		for _query in enumerate(queries, 0):
			endpoint = "https://haveibeenpwned.com/api/v3/breachedaccount/{}".format(_query[1])
			async with self.aiohttp.get(endpoint, headers={"hibp-api-key": tragedy.DotenvVar("hibpKey"), "User-Agent": "tragedy"}) as _response:
				try:
					jObj = await _response.json()
					description = '\n'.join([breach.get("Name") for breach in jObj])
				except aiohttp.ContentTypeError:

    ...
    async def view(self, ctx):
        ...
        try:
            ...
            embed = discord.Embed(title="Tragedy temporary email",
                                  description="You do not have a tragedy temporary email setup yet", color=Color.red())
            return await ctx.send(embed=embed)
        async with self.aiohttp.get("https://www.1secmail.com/api/v1/?action=getMessages&login=%s&domain=%s" % (email.split('@')[0], email.split('@')[1])) as request:
            jObj = await request.json()
            SelectOptions = []
            for _index in range(len(jObj)):
                SelectOptions.append(
                    SelectOption(label='%s (%s)' % (jObj[_index]['subject'], jObj[_index]['from']), value=jObj[_index]['id']))
            try:
            ...
            async with self.aiohttp.get("https://www.1secmail.com/api/v1/?action=readMessage&login=%s&domain=%s&id=%s" % (email.split('@')[0], email.split('@')[1], selectInteraction.component[0].value)) as request:
                jObj = await request.json()
            ...
            if jObj['htmlBody'] == "":
                if len(jObj['textBody']) > 1500:
                    await ctx.send(file=discord.File(fp=io.BytesIO(str(jObj['textBody']).encode(errors='ignore')), filename="email.txt"))
                else:
                ...
                return await ctx.author.send(embed=embed, file=discord.File(fp=io.BytesIO(str(jObj['htmlBody']).encode(errors='ignore')), filename="body.html"))
            await SelectMail.delete()
            await ctx.message.add_reaction("✅")

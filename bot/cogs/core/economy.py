...
class Economy(commands.Cog, description="Economy system lol"):
    ...
    async def get_user_balance(self, guild_id: int, user_id: int) -> int:
        with self.pool.cursor() as cursor:
            try:
                cursor.execute(
                    "SELECT balance FROM `economy` WHERE guild=%s AND user=%s", (guild_id, user_id))
    ...
    async def get_user_items(self, guild_id: int, user_id: int) -> dict:
        with self.pool.cursor() as cursor:
            try:
                cursor.execute(
                    "SELECT items FROM `economy` WHERE guild=%s AND user=%s", (guild_id, user_id))
                row: dict = cursor.fetchone()
    ...
    async def add_user_balance(self, guild_id: int, user_id: int, amount: int) -> int:
        with self.pool.cursor() as cursor:
            try:
                cursor.execute(
                    "UPDATE `economy` SET balance = balance + %s WHERE guild=%s AND user=%s", (amount, guild_id, user_id))
                return amount
            except AttributeError:
                cursor.execute(
    ...
    async def buy_item(self, guild_id: int, user_id: int, item: str, amount: typing.Optional[int] = 1) -> bool:
        ...
        if price < balance:
            with self.pool.cursor() as cursor:
                try:
                    cursor.execute("UPDATE `economy` SET balance = balance - %s, items = JSON_SET(items, %s, %s + %s) WHERE guild = %s AND user = %s",
                                   (price, ("$." + item), count, amount, guild_id, user_id))
                    return True
                except AttributeError:
                    pass
        else:
    ...
    async def bal(self, ctx: commands.Context, *, member: typing.Optional[commands.MemberConverter]):
        ...
        embed = discord.Embed(title="%s's Financial Funds $$" % (member.name), color=Color.green(
        ), description="**Wallet**: \U0001FA99 %s" % (humanize.intcomma(balance)))
    ...
    async def inv(self, ctx: commands.Context):
        ...
        embed = discord.Embed(title="Your personal belongings",
                              color=Color.green(), description=full_inventory)
        await ctx.send(embed=embed)
    ...
    async def beg(self, ctx: commands.Context):
        ...
        embed = discord.Embed(title="Imagine begging lol", color=Color.green(
        ), description="You came by a very generous soul who gave you \U0001FA99 **%s** !" % (str(money)))
        await ctx.send(embed=embed)
    ...
    async def buy(self, ctx: commands.Context, item: str, amount: typing.Union[int, str] = 1):
        ...
        if await self.buy_item(ctx.guild.id, ctx.author.id, item.casefold()):
            embed = discord.Embed(title="Tragedy shop", color=Color.green(
            ), description="You just bought **%s** `%s` for \U0001FA99 **%s**" % (str(amount), item, self.shop.get(item.casefold()).price))
            await ctx.send(embed=embed)
    ...
    async def shop(self, ctx: commands.Context):
        ...
        for item in self.shop:
            shop += "%s **%s** - `%s`\n" % (self.shop.get(item).emoji,
                                            item, self.shop.get(item).price)
        try:
